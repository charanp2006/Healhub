  ...
  return (
    <div className='flex justify-between items-center text-sm py-4 mb-5 border-b border-b-gray-400'>
        <div onClick={()=>navigate('/')} className='flex items-center gap-2 cursor-pointer'>
          ...
          <span className='text-2xl font-bold text-gray-800'>Healhub</span>
        </div>
        <ul className='md:flex items-start gap-5 font-medium hidden'>
            ...
            <NavLink to='/hospitals'>
                <li className='py-1'>Search Hospitals</li>
                <hr className='border-none outline-none h-0.5 bg-primary w-3/5 m-auto hidden' />
            </NavLink>
        </ul>
            ...
            {/* ------- Mobile menu ------- */}
              <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                ...
                <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
                    ...
                    <NavLink onClick={()=>setShowMenu(false)} to='/hospitals'><p className='px-4 py-2 rounded inline-block'>Search Hospitals</p></NavLink>
                </ul>
            </div>
        </div>
    </div>
