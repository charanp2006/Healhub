    ...
    <form onSubmit={onSubmitHandler} className="m-5 w-full">
      ...
      <div className="bg-background-card px-8 py-8 rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll">
        ...
        <div className="flex gap-3 mt-4">
          ...
          {editId && <button type="button" onClick={() => router.push("/blogs-list")} className="px-10 py-3 border border-border rounded-full cursor-pointer hover:bg-primary-soft transition-colors">Cancel</button>}
        </div>
      </div>
    </form>
  );
